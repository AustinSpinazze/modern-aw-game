...
async function runQuickCheck(): Promise<CheckResult> {
  ...
  try {
    ...
    // Take screenshot
    const screenshotPath = path.join(RESULTS_DIR, "quickCheck.png");
    ...
    const textPath = path.join(RESULTS_DIR, "quickCheck-text.txt");
...
(async () => {
  ...
  const jsonPath = path.join(RESULTS_DIR, "quickCheck-result.json");
