...
## Group 2: Roads (11 unique tiles)

**Prompt:**

> [Style preamble]
>
> Generate a horizontal sprite strip of 11 road tile variants for a turn-based tactics grid game. Each tile is 16x16 pixels with a plains/grass background and a grey paved road path drawn on top. The road path is about 6-8 pixels wide, centered, with a 1px dark grey border on each side of the road surface. Road color: medium grey (#888888 range) with lighter grey center line.
>
> Roads connect at tile edges using a 4-directional system: North (top), East (right), South (bottom), West (left). The game engine uses a 4-bit bitmask to select tiles — single-direction and isolated cases reuse the straight tiles, so only these 11 unique shapes are needed.
>
> Generate them left to right in this exact order:
>
> **Straights (2):**
>
> 1. Vertical straight (exits top and bottom)
> 2. Horizontal straight (exits left and right)
>
> **Corners (4):** 3. Corner: top + right (L-turn) 4. Corner: right + bottom (L-turn) 5. Corner: bottom + left (L-turn) 6. Corner: top + left (L-turn)
>
> **T-junctions (4):** 7. T-junction: top + right + bottom (open on left side) 8. T-junction: right + bottom + left (open on top) 9. T-junction: top + bottom + left (open on right) 10. T-junction: top + right + left (open on bottom)
>
> **Crossroads (1):** 11. 4-way crossroads (exits all 4 directions)
>
> Export as a single PNG strip, exactly 176 pixels wide (11 tiles × 16px) and 16 pixels tall.
>
...
## Group 3: Rivers (11 unique tiles)

**Prompt:**

> [Style preamble]
>
> Generate a horizontal sprite strip of 11 river tile variants. Same 4-directional system as the road set. Each tile is 16x16 pixels.
>
...
>
> Generate the same 11 directional variants as the roads (2 straights, 4 corners, 4 T-junctions, 1 crossroads) in the same order. The water channels must align at tile edges exactly like the roads do.
>
> Export as a single PNG strip, exactly 176 pixels wide (11 tiles × 16px) and 16 pixels tall.
>
...
1. **Terrain**: Combine Groups 1–4 (basic terrain, roads, rivers, bridges + shoals) into a single `awbw-terrain.png` atlas. Generate the matching JSON atlas.
2. **Neutral buildings**: Group 5 goes into `neutral.png` with matching JSON atlas.
