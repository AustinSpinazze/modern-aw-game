...
### Group 2: Roads (16 variants, 16x16 each)

Auto-tiled via 4-bit bitmask (N=1, E=2, S=4, W=8). Each road tile has **plains background with a grey road path**. Roads connect smoothly at tile edges.
...
### Group 3: Rivers (16 variants, 16x16 each)

Identical bitmask structure as roads. Blue water channel with green grass banks. Rivers connect to sea and port tiles at edges.
...
Each building also has **rain** and **snow** weather variants (same frame count, suffix `-rain-` and `-snow-`), but these are low priority.
