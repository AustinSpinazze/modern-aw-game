...
export default function BuyMenu({ facilityX, facilityY, onClose }: BuyMenuProps) {
  ...
  return (
    <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-96 max-h-[80vh] flex flex-col">
        {/* Header */}
          ...
          {producible.map((unitType) => {
            ...
            return (
              <div key={unitType} className={`${canBuy ? "" : "opacity-40"}`}>
                <div
                  className={`flex items-center px-4 py-3 gap-3 ${canBuy ? "cursor-pointer hover:bg-slate-800/60 transition-colors" : "cursor-not-allowed"}`}
                  onClick={() => canBuy && setExpanded(isExpanded ? null : unitType)}
                >
                  {/* Unit info */}
                  <div className="flex-1 min-w-0">
                    <div className={`font-semibold ${canBuy ? "text-white" : "text-slate-500"}`}>
                      {unitData.name}
                    </div>
                  ...
                  <div className="text-right shrink-0">
                    <div
                      className={`font-mono font-bold text-sm ${canBuy ? "text-amber-400" : "text-slate-600"}`}
                    >
                      ¥{unitData.cost.toLocaleString()}
                    </div>
                    {isExpanded && <div className="text-slate-500 text-xs">▲ Less</div>}
                    {!isExpanded && <div className="text-slate-600 text-xs">▼ More</div>}
                  </div>
                </div>
                ...
                {isExpanded && (
                  <div className="px-4 pb-3 bg-slate-800/60 space-y-2">
                    ...
                    {unitData.weapons.length === 0 && (
                      <div className="text-xs text-slate-500 italic">No weapons (transport)</div>
                    )}
                    {primaryWeapon && (
                      <div className="text-xs">
                        ...
                        {primaryWeapon.ammo > 0 && (
                          <span className="text-slate-500 ml-1">({primaryWeapon.ammo} ammo)</span>
                        )}
                    ...
                    <div className="grid grid-cols-3 gap-2 text-xs pt-1">
                      <div>
                        <div className="text-slate-500 uppercase tracking-wide">Move</div>
                        <div className="text-white font-medium">{unitData.move_points}</div>
                      </div>
                      <div>
                        <div className="text-slate-500 uppercase tracking-wide">Type</div>
                        <div className="text-white font-medium capitalize">
                          {unitData.move_type}
                        </div>
                      </div>
                      <div>
                        <div className="text-slate-500 uppercase tracking-wide">Vision</div>
                    ...
                    <button
                      ...
                      className="w-full mt-1 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black py-2.5 rounded-lg transition-colors text-sm"
                    >
        ...
        <div className="px-4 py-3 border-t border-slate-700 shrink-0">
          <button
            onClick={onClose}
            className="w-full bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 py-2 rounded-lg transition-colors text-sm"
          >
            Cancel
          </button>
        </div>
